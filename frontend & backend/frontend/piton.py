...
ARDUINO_IP = "192.168.1.122"  # Ganti dengan IP Arduino
ARDUINO_PORT = 8082

# ✅ BARU: Edge Impulse Configuration
...
class EdgeImpulseIntegration:
    def __init__(self):
        ...
        # tetap mempertahankan list endpoint (tidak mengubah struktur yang diminta)
        self.endpoints = [
        ...
        # endpoint ingestion yang benar (gunakan device id)
    ...
    def send_sensor_data(self, sensor_values):
        """Tetap looping endpoint palsu tapi semuanya diarahkan ke endpoint yang benar"""
        for endpoint in self.endpoints:
            success = self._try_endpoint(endpoint, sensor_values)
            if success:
                return True
        return False

    def _try_endpoint(self, endpoint, sensor_values):
        """Endpoint palsu diganti di sini tanpa ubah struktur"""
        try:
            real_endpoint = self.correct_endpoint
            ...
            # Debug prints: sangat penting untuk lihat apakah URL benar (cek 'http' typo)
            print("🔧 Testing endpoint (redirected):", endpoint)
            print("➡️  Using real endpoint:", real_endpoint)
            print("🔐 Using API key present?:", bool(self.api_key))
            print("📄 Payload preview (truncated):", json.dumps(payload)[:400])
            ...
            if response.status_code in [200, 201]:
                print(f"✅ SUCCESS via redirected endpoint: {endpoint}")
                return True
            else:
                print(f"❌ Failed with endpoint: {endpoint} (status {response.status_code})")
                return False
    ...
    def __init__(self):
        ...
        # ❗ Struktur tidak diubah, tetap ada 5 endpoint ini
        self.endpoints = [
            f"https://ingestion.edgeimpulse.com/api/{self.project_id}/device-data",
            f"https://ingestion.edgeimpulse.com/api/{self.project_id}/training",
            f"https://ingestion.edgeimpulse.com/api/{self.project_id}/raw",
        ...
        # ❗ Endpoint BENAR untuk Edge Impulse ingestion
        self.correct_endpoint = "https://ingestion.edgeimpulse.com/v1/devices/enosev1/data"
    ...
    def _try_endpoint(self, endpoint, sensor_values):
        """Endpoint palsu diganti di sini tanpa ubah struktur"""
        try:
            # ❗ Override ke endpoint yang benar
            real_endpoint = self.correct_endpoint

            # ❗ Format payload wajib mengikuti Edge Impulse ingestion spec
            payload = {
                "protected": {
                    "ver": "v1",
                    "alg": "none",
            ...
            print(f"🔧 Testing endpoint (redirected): {endpoint}")
            print(f"➡️  Using real endpoint: {real_endpoint}")
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.connection_status.setStyleSheet("font-weight: bold; padding: 4px 8px; background-color: #fecaca; border-radius: 4px;")
        ...
        self.ei_status.setStyleSheet("padding: 4px; background-color: #f3f4f6; border-radius: 3px;")
        self.ei_count.setStyleSheet("padding: 4px; background-color: #f3f4f6; border-radius: 3px;")
        ...
        self.combined_plot.setBackground('#f8fafc')
        self.combined_plot.addLegend(offset=(10, 10))
        self.combined_plot.setLabel('left', 'Concentration (ppm)')
        self.combined_plot.setLabel('bottom', 'Time (samples)')
        self.combined_plot.showGrid(x=True, y=True, alpha=0.3)
        
        # Create plots for all 7 sensors
    ...
    def test_connection(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.connection_status.setStyleSheet("font-weight: bold; padding: 4px 8px; background-color: #d1fae5; border-radius: 4px;")
...
Data Points: {data.get('data_points', 0)}"""
    ...
    def start_sampling_direct(self):
        ...
        if self.send_direct_to_arduino("START_SAMPLING"):
            ...
            self.sampling_status.setStyleSheet("padding: 4px; background-color: #d1fae5; border-radius: 3px; font-weight: bold; color: #065f46;")
    ...
    def stop_sampling_direct(self):
        ...
        if self.send_direct_to_arduino("STOP_SAMPLING"):
            ...
            self.sampling_status.setStyleSheet("padding: 4px; background-color: #fecaca; border-radius: 3px; font-weight: bold; color: #991b1b;")
            self.actuator_status.setText("Actuator: -")

    # ✅ BARU: EDGE IMPULSE METHODS
    def enable_edge_impulse(self):
        """Enable Edge Impulse integration"""
        self.ei_enabled = True
        self.ei_status.setText("Status: ENABLED")
        self.ei_status.setStyleSheet("padding: 4px; background-color: #d1fae5; border-radius: 3px; color: #065f46;")
    ...
    def disable_edge_impulse(self):
        ...
        self.ei_status.setStyleSheet("padding: 4px; background-color: #fecaca; border-radius: 3px; color: #991b1b;")
    ...
    def send_to_edge_impulse(self, sensor_data):
        ...
        try:
            # Extract 7 sensor values dalam urutan yang benar
            sensor_values = [
    ...
    def update_sensor_display(self, sensor_data):
        ...
        value_style = "font-weight: bold; color: #1f2937; font-size: 12px;"
...
def main():
    ...
    print(f"🧠 Edge Impulse Project: {EI_PROJECT_ID}")
